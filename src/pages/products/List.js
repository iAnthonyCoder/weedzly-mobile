...
const ProductList = () => {
    ...
    const [ 
		...
		service: productService.getPaginated,
		includeLocation: true,
		sortingOptions: sorting_settings.strainsPage,
        startWithUrlFilter: true
	})
    ...
    useEffect(() => {
        filterer(window.location.search.length > 0 ? window.location.search.replace('?', '&') : '?')
    }, []);
    ...
    return (
        <IonPage>
            ...
            />
            <IonHeader>
                <TopToolbar title={'Products'}></TopToolbar>
            </IonHeader>
